...
| `duster mcp list \| show \| sync \| rm \| ping` | 每条声明一行，STATE 列说清跨 agent 的合并结论：所有声明者一致 = `identical`、不一致 = `drifted`、只有一家声明 = `only copy`——差异直接摆在表里，不再需要单独的 diff 命令。`show` 打印全文，env 值与 header 一律掩码。`sync <name> --to codex,opencode` 把一条声明复制进别的 agent 并转换格式；默认只出计划，写之前对每个文件做整文件快照，目标格式装不下的字段一律**拒写**。`rm <name> --agent <A>` 从**那个 agent 自己的主配置文件**（`~/.claude.json`、`~/.codex/config.toml`、`~/.gemini/settings.json` …）里摘掉这一条声明——那是**你自己的主配置**，不是 duster 的：文件的形状漂移时由同一道 schema_guard 拒写，只动那一个键（文件里其余内容逐字节不变）。这是真删，不打归档包——退路是**改写之前留下的整文件快照**（落在 `~/.agent-duster/snapshots/`）：这里有风险的是「一份还装着二十条别的东西的文件被写坏」，不是「丢了一条声明」。`--dry-run` 打印哪个文件的哪个键会没，`--all-agents` 删除该名字的每一条声明。`ping` 把每个 server 起一次，说一句 `initialize` 就挂断。 |
| `duster skill list \| link \| rm` | 跨 agent skill 管理。`list` 列出你有的全部 skill，并标出哪几个装在多处——同一个 agent 里的两份也算——以及那几份里哪些已经漂移；`link` 把重复项收敛到磁盘上的同一份——一处修改，全局生效。`rm <name> --agent <A> [--path <P>]` 精确删除一份副本，**真删，不打包**——同名多家时 `--agent` 必选，同一家装了多份（目录不同）时 `--path` 再必选，duster 绝不替你默认删全部；软链副本只 unlink 链接本身（指向的内容原样留在别处）。是你点的名、你确认的；按龄批量清 skill 的是 `duster prune`，那一条永远先打包。 |
...
| `duster prune --older-than 30d` | 清**陈旧的用户资源**：N 天没改过、也没有任何**调用记录**的 skill（名字在正文里被提到一句不算证据，见下），过期的备份与归档；N 天以上的会话改为压缩存档（不删内容）。加 `--keep-generations` 还会清掉数据库备份这类「一代一份」资源的超编副本——它们冗余是因为份数多，不是因为旧。一定先逐条列出「是什么 / 为何判定陈旧 / 删了什么后果」，确认后永久删除；不可再生的内容**一律先打包到 `~/agent-duster-exports/`，没有关掉它的旗标**：prune 清的是你没有逐条过目的东西，那正是一条退路值得占这点磁盘的场合。 |
...
菜单里的 `uninstall` 单选一个 agent（不给勾选表——一屏勾多个 agent 全删，这个批量语义
太危险，产品上不提供），然后问一张**两格的范围表**：**配置与数据**（`~/.qoder`——skills、
MCP 声明、会话、记忆）默认勾上，**软件本体**默认不勾——勾它等于把控制权交给你的包管理器，
默认替人做主是错的。接着只有一道 y/N，把 agent 名、体积、条数一次说全，并明说
**什么都不归档、不可撤销**：菜单这条路是真删干净的。来卸载一个 agent 的人，不是来把同一批
字节挪进一个他还得自己去清的目录的。想要退路的走命令行：那边 `--export-first` 默认开，
`--keep sessions,memory` 让它们留在原地。
...
### 归档给谁：只给会话和记忆，且只在你没点名的时候

说清楚这件事，因为含糊过去很容易变成一句漂亮话：**归档包就是一份回收站**。
内容没消失，它去了 `~/agent-duster-exports/`，而且得由你来清。所以问题不是
「有没有回收站」，而是「哪些删除配得上一份回收站」。duster 的答案有两条轴。

**第一条轴：东西丢了还能不能拿回来。**

| 内容 | 删的时候 |
|---|---|
| 缓存、SQLite 空闲页、孤儿 WAL、日志（`clean`，本机 774.6 MB，占绝大头） | 真删，一个副本不留。缓存的撤销毫无意义；何况 VACUUM 与日志截断都是原地改写，**根本不存在一个可以被移走的文件** |
| skill、MCP 声明（`skill rm` / `mcp rm`） | 真删。skill 你还有原始来源，MCP 那条声明就是几行 JSON——为它们养一个回收站，等于让你以后再清一次 |
| 会话、记忆（`session rm` / `memory rm`） | **默认先打包**。这是唯一真正不可再生的东西：一场对话删了就是删了 |

一个清理工具跑完 `df` 没变化，是它能犯的最严重的错误。上面第一、第二行就是
为了让 `df` 真的变。

**第二条轴：这一刀是你点的名，还是 duster 按龄挑的。**

- **你点了名**（`rm` 系列）：显式行为不该暗中留副本。`skill rm` 要求 `--agent`、
  同一家装了多份还要 `--path`，交互里还有 y/N——你已经看清楚在删什么了。
  会话和记忆在详情屏里给**两个并列选项**：归档后删、真删。默认是前者，但
  你明确知道自己在扔什么的时候，工具不该硬塞给你一个还得自己清的包。
- **duster 按龄挑的**（`prune`）：**恒归档，没有关掉的旗标**。批量清理的是你
  没有逐条过目的东西，那正是回收站唯一站得住的场合。

**那个包归你，不归 duster。** 一个 `.tar.zst`，`tar -xf` 就是还原。duster 不 GC
它、不提供 restore 命令、不在任何视图里管理它。它不会哪天自己把你的东西清掉，
代价是也不会自己变小：**它只增不减**，`duster doctor` 会报出它现在几个包、
多少字节，剩下的你自己定。压得很狠——全部 77 个 skill 的用户内容打包后 7.9 MB。
...
- **会话不是"删"是"压"。** 解压后逐字节校验与原文件一致，才会删原件。
  可验证的正确性比可撤销更强。
- **改配置文件不走归档，走快照。** 从 `~/.claude.json` 里摘掉一条 MCP 不是删文件，
  是改一个还装着二十条别的东西的文件——这种操作前会做整文件快照，落在
  `~/.agent-duster/snapshots/` 下。改坏了从那儿捞，与归档是两回事。
...
3. **默认只读** — 破坏性操作必须显式开启；删除前一定逐条列清单，凡是 prune 按龄扫到的一律先归档。
