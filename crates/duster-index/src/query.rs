...
impl ResourceRecord {
    ...
    /// 「上次使用」的证据,取 mtime;**没有证据时是 `None`,不是 1970**。
    ///
    /// `mtime_ns <= 0` 表示 scan 当时没取到时间戳(stats-only 采集失败、
    /// 权限不足等)。把它当成「1970 年用过」会让这一行在任何陈旧判定下
    /// 直接判死,也会在列表里印成「56 年前」——两者都是凭空捏造。
    ///
    /// 这条规则只许有一份实现:`prune` 的陈旧判定(`plan` 的 mtime 证据)
    /// 与四张列表的 LAST USED 列都从这里取,否则「列表说它上周用过、prune
    /// 说它没有时间戳」这种自相矛盾迟早出现。
    ///
    /// 语义上它是**最后修改**时间,不是「最后被 agent 加载」——磁盘上只有
    /// 这一个可得的信号。渲染方要按这个含义措辞,不要许诺调用记录。
    pub fn last_used_ms(&self) -> Option<i64> {
        (self.mtime_ns > 0).then(|| self.mtime_ms())
    }
}
